                 ...
                 @foreach ($anuncio->imagens->sortBy('position') as $img)
                     <div class="col-lg-2 ">
                        ...
                        <p class="text-sm mt-2">
                            <input type="hidden" name="imagen[{{ $img->id }}][id]" id="imagen{{ $img->id }}" value="{{ $img->id }}"  class="col-sm-4">
                            <strong>Nro:</strong> 
                            <input type="number" name="imagen[{{ $img->id }}][posicion]" id="imagen{{ $img->position }}" value="{{ $img->position }}" class="col-sm-4"><br>
                            
                            <strong>Portada Solicitada:</strong> {{ $img->portada }}<br>
                            
                            <input type="radio" name="portada_id" id="portada_id_{{ $img->id }}" value="{{ $img->id }}"  {{ $anuncio->portada_id == $img->id ? 'checked' : '' }}>
                            <label for="portada_id_{{ $img->id }}"><strong>Portada</strong></label><br>
                            <strong>Estado:</strong> {{ $img->estado }}
                        </p>
                         {{-- <p class='text-sm mt-2'>
