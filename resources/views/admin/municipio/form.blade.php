<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    <div class="space-y-6">
        ...
        <div class="row">
            ...
            <div class="form-group col-md-6 space-y-2">
                ...
                <select id="provincia_id"
                        name="provincia_id"
                        class="form-control block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('provincia_id') border-red-500 @enderror">
                    @foreach($provincias as $id => $provincia)
                        <option value="{{ $id }}" @selected(old('provincia_id', $municipio->provincia_id) == $id)>
                            {{ $provincia }}
                        </option>
                    @endforeach
                </select>
       ...
       <div class="row">
           ...
           <!-- Isla Field -->
           <div class="form-group col-md-6 space-y-2">
               <label for="isla_id" class="block text-sm font-medium text-gray-700">Isla</label>
               <select id="isla_id"
                       name="isla_id"
                       class="form-control block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('isla_id') border-red-500 @enderror">
                   <option value="">Seleccione una isla</option>
                   @foreach($islas as $id => $isla)
                       <option value="{{ $id }}" @selected(old('isla_id', $municipio->isla_id) == $id)>
                           {{ $isla }}
                       </option>
                   @endforeach
               </select>
        ...
        <div class="row">
            ...
            <div class="space-y-2">
                ...
                <textarea id="texto_seo"
                          name="texto_seo"
                          rows="4"
                          class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('texto_seo') border-red-500 @enderror"
